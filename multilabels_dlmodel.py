...
def GRU_NETWORK():
    ...
    model.add(Embedding(input_dim=token_words, input_length=max_lenth, output_dim=32))
    model.add(Dropout(0.2))  # 加入Dropout层，防止过拟合
    model.add(GRU(units=64))     # 加入Flattern层，变为３２００个神经元
...
if __name__ == '__main__':
    ...
    # joblib.dump(allBunch, ALL_BUNCH_FILE_DL) # 保存train_bunch和test_bunch,后面的程序可以直接读取，以节省时间。
    # joblib.dump(allBunch, ALL_BUNCH_FILE_DL) # 保存train_bunch和test_bunch,后面的程序可以直接读取，以节省时间。
    allBunch = joblib.load(ALL_BUNCH_FILE) # 读取保存的allBunch
    # 先不进行特征选择对allBunch进行交叉验证，多标签训练
    trainBunch = Bunch(labels=[], new_labels=[],contents=[], vector=[],  selectVector=[], tfidf=[], selectTfidf=[])
    testBunch = Bunch(labels=[], new_labels=[],contents=[], vector=[],  selectVector=[], tfidf=[], selectTfidf=[])
    trainBunch.new_labels, testBunch.new_labels, trainBunch.contents, testBunch.contents\
        = train_test_split(allBunch.multi_labels, allBunch.contents,test_size=0.3)
    # 相关参数如下：
