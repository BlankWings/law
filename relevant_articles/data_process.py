...
def gen_parameters():
    ...
    return lawlist, stopwordslist, train_data_list, test_data_list
...
def writeData(json_path, data_path):
    ...
    with open(json_path, "r", encoding="utf-8") as f:
        for line in f.readlines():
            ...
            law_list = line["meta"]["relevant_articles"]
            for law in law_list:
                filename =  os.path.join(data_path, str(law))+ "/raw_data.txt"
...
if __name__ == '__main__':
    ...
    TRAIN_DATA = os.path.join(BASEPATH, "data/relevant_articles/train_data")
    TEST_DATA = os.path.join(BASEPATH, "data/relevant_articles/test_data")
    ALL_DATA = os.path.join(BASEPATH, "data/relevant_articles/all_data")
    LAW_FILE = os.path.join(BASEPATH, "data/raw_data/law.txt")
    STOPWORDS_FILE = os.path.join(BASEPATH, "data/stop_words/stopwords.txt")
    SEG_NAME = "seg_data.txt"
    PROCESS_NAME = "process_data.txt"

    # 生成相关参数
    lawlist, stopwordslist, train_data_list, test_data_list = gen_parameters()

    # 读取JSON文件
    st = time.time()
    writeData(TRAIN_DATA_JSON, TRAIN_DATA)
    writeData(TEST_DATA_JSON, TEST_DATA)
    et = time.time()
    time0 = et - st
