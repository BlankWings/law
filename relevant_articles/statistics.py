...
if __name__ == '__main__':
    # 文件目录
    BASE_DIR = os.getcwd()
    RAW_DATA_FILE = os.path.join(os.path.dirname(BASE_DIR),"data/raw_data/data_train.json")
    RESULT_PATH = os.path.join(os.path.dirname(BASE_DIR), "data/multi_labels")
    STATISTICS_PATH = os.path.join(os.path.dirname(BASE_DIR), "data/statistics")
    ...
    PROBABILITY_FILE  = os.path.join(STATISTICS_PATH, "probability.csv")
    NUMBER_FILE  = os.path.join(STATISTICS_PATH, "number.csv")
    ACCU_FILE = os.path.join(os.path.dirname(BASE_DIR),"data/raw_data/accu.txt")
    ...
    pro_metrix.to_csv(PROBABILITY_FILE)
    num_metrix.to_csv(NUMBER_FILE)

    # 读取已经生成的csv文件，找到一些数量和比例比较高的类别
    # pro_metrix = pd.read_csv(PROBABILITY_FILE)
    # num_metrix = pd.read_csv(NUMBER_FILE)
    # for i in pro_metrix[:]:
    #     for j in pro_metrix[:]:
